...
const navigate = useNavigate();
  ...
  const fetchStudents = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        per_page: (perPage === -1 ? (totalStudents > 0 ? totalStudents : 100000) : perPage).toString(),
        sort_by: sortBy,
        sort_dir: sortDir
      });
      ...
      const response = await privateAxios.get<ApiResponse>(`/students/list?${params}`);
  ...
  subjects: s.subjects ?? null
      ...
      setCurrentPage(prev => Math.min(prev, Math.max(1, data.meta.total_pages || 1)));

      setFiltersMeta(data.filters_meta || { years: [], genders: [], branches: [] });
  ...
  'branch', 'year_of_study', 'semester', 'cgpa', 'gender',
  'is_active', 'first_time_login',
  'address', 'city', 'state', 'pincode',
  'guardian_name', 'guardian_contact', 'subjects'
];
...
students.forEach(s => {
  const row = [
    ...
    escapeCsv(Array.isArray(s.subjects) ? (s.subjects.join(';')) : (s.subjects ?? ''))
  ];
  ...
  const startIndex = totalStudents === 0 ? 0 : ((currentPage - 1) * (perPage === -1 ? totalStudents : perPage)) + 1;
  const endIndex = perPage === -1 ? totalStudents : Math.min(currentPage * perPage, totalStudents);
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-8">
          <h3 >{collegeName} • {totalStudents} students</h3>
        </div>
    {/* Search and Filters */}
                  ...
                  <thead className="bg-gray-50 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Student</th>
                                            <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">USN / Enrollment Number</th>
  ...
  Actions
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {students.map((student,idx) => {
                      const initials = (student.name || '').split(' ').filter(Boolean).map(n => n[0]).join('').slice(0,2).toUpperCase() || (student.email ? student.email[0].toUpperCase() : '--');
...
{(student.address || student.pincode) && (
  <div className="text-sm text-gray-500 flex items-center gap-1">
    ...
    {student.address ? `${student.address}${student.city ? ', ' + student.city : ''}${student.state ? ', ' + student.state : ''}` : ''}
                          ...
                          <td className="px-6 py-4">
                            <div className="space-y-1">
                              ...
                              <div className="text-sm text-gray-500 flex items-center gap-1">
                                ...
                                {student.year_of_study ? `Year ${student.year_of_study}` : 'Year —'}, {student.semester ? `Sem ${student.semester}` : 'Sem —'}
                              </div>
                         
                              <div className="text-xs text-gray-500 flex items-center gap-1">
                                <User className="w-3 h-3" /> Gender: {student.gender ?? '—'}
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-1">
                              ...
                              <span className="text-sm font-medium text-gray-900">
                                {Number.isFinite(cgpaNumber) ? cgpaNumber.toFixed(2) : 'N/A'}
                              </span>
                            </div>
                          </td>
                          <td className="px-6 py-4">
