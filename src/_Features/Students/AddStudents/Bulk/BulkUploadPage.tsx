...
const BulkUploadPage: React.FC = () => {
  const [currentStep, setCurrentStep] = useState<'upload' | 'preview' | 'mapping' | 'complete'>('upload');
  ...
  const handleProcessData = async () => {
    ...
    const mappedData = excelData.rows.map(row => {
      ...
      Object.entries(columnMapping).forEach(([excelColumn, targetField]) => {
        ...
        if (columnIndex !== -1) {
          mappedRow[targetField] = row[columnIndex];
        }
      });
      return mappedRow;
    });
    ...
    try {
      ...
      if (mode === 'insert') {
        ...
        resp = await privateAxios.post<ApiResponseShape>('/students/add-bulk-students', { mappedData });
      } else {
        ...
        resp = await privateAxios.post<ApiResponseShape>('/students/upsert-bulk-students', {
      ...
      const msg = err?.response?.data?.message || err?.message || 'Failed to Process Data';
      showError(msg);
    } finally {
      setIsProcessing(false);
    }
  ...
  const steps = [
    { id: 'upload', label: 'Upload File', icon: Upload, completed: currentStep !== 'upload' },
    { id: 'preview', label: 'Preview Data', icon: Database, completed: ['mapping', 'complete'].includes(currentStep) },
    { id: 'mapping', label: 'Map Columns', icon: Settings, completed: currentStep === 'complete' },
    { id: 'complete', label: 'Complete', icon: CheckCircle, completed: currentStep === 'complete' }
  ];

  const statusBadge = (status: string) => {
    const base = "inline-flex items-center px-2 py-1 rounded-full text-sm font-medium";
    if (status === 'created') return <span className={`${base} bg-green-100 text-green-800`}>Created</span>;
    if (status === 'created_email_failed') return <span className={`${base} bg-yellow-100 text-yellow-800`}>Created (email failed)</span>;
    if (status === 'updated') return <span className={`${base} bg-blue-100 text-blue-800`}>Updated</span>;
    if (status === 'skipped') return <span className={`${base} bg-gray-100 text-gray-800`}>Skipped</span>;
  ...
  const downloadFile = (filename: string, content: Blob | string, mime = 'application/octet-stream') => {
  ...
  const downloadCSV = () => {
    ...
    const headers = ['index', 'status', 'message', 'email', 'student_id', 'provided'];
    const lines = [];
    ...
    downloadFile(`bulk-upload-results-${ts}.csv`, csvContent, 'text/csv;charset=utf-8;');
  };
  ...
  const downloadJSON = () => {
    ...
    downloadFile(`bulk-upload-results-${ts}.json`, pretty, 'application/json;charset=utf-8;');
  };
            ...
            return (
              <div key={step.id} className="flex items-center">
                ...
                <span className={`ml-3 font-medium ${
                  isActive ? 'text-[#4CA466]' : isCompleted ? 'text-[#4CA466]' : 'text-gray-500'
                }`}>
                  {step.label}
                </span>
                {index < steps.length - 1 && (
                  <div className={`w-16 h-0.5 mx-6 ${ step.completed ? 'bg-[#4CA466]' : 'bg-gray-300' }`} />
                )}
              </div>
            );
          })}
                    ...
                    {apiResponse
                      ? (
                          apiResponse.updated_count !== undefined
                            ? `Processed ${apiResponse.total_received} records — ${apiResponse.created_count ?? 0} created, ${apiResponse.updated_count ?? 0} updated.`
                            : `Processed ${apiResponse.total_received} records — ${apiResponse.created_count ?? 0} created.`
                        )
            ...
            {apiResponse && (
              <div className="space-y-6">
                ...
                <div className="rounded-md border p-4">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <div className="font-medium">{apiResponse.college?.name || '-'}</div>
                      <div className="text-xs text-gray-400">{apiResponse.college?.id}</div>
                    </div>

                    <div className="text-right">
                      <div className="text-sm text-gray-500">Total received</div>
                      <div className="font-medium">{apiResponse.total_received}</div>
                      <div className="text-sm text-gray-500">Created</div>
                      <div className="font-medium">{apiResponse.created_count ?? 0}</div>
                      {apiResponse.updated_count !== undefined && (
                        <>
                          <div className="text-sm text-gray-500 mt-2">Updated</div>
                          <div className="font-medium">{apiResponse.updated_count ?? 0}</div>
                        </>
                      )}
                    </div>
                  </div>
                  ...
                  <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-100">
                      {apiResponse.results.map((r) => (
                        <tr key={r.index}>
                          <td className="px-4 py-3 text-sm text-gray-700">{r.index + 1}</td>
                          <td className="px-4 py-3 text-sm">{statusBadge(r.status)}</td>
                          <td className="px-4 py-3 text-sm text-gray-700">{r.message || (r.provided ? JSON.stringify(r.provided) : '-')}</td>
                          <td className="px-4 py-3 text-sm text-gray-700">{r.email || '-'}</td>
                          <td className="px-4 py-3 text-sm text-gray-700">{r.student_id || '-'}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-sm text-gray-600">
                    {apiResponse.results.filter(r => r.status === 'created').length} created
                  </div>
                  <div className="text-sm text-gray-600">
                    {apiResponse.results.filter(r => r.status === 'created_email_failed').length} created (email failed)
                  </div>
                  <div className="text-sm text-gray-600">
                    {apiResponse.results.filter(r => r.status === 'updated').length} updated
                  </div>
                  <div className="text-sm text-gray-600">
                    {apiResponse.results.filter(r => r.status === 'skipped').length} skipped
                  </div>
                  <div className="text-sm text-gray-600">
                    {apiResponse.results.filter(r => !['created','created_email_failed','updated','skipped'].includes(r.status)).length} errors
                  </div>
                </div>
              </div>
            )}
