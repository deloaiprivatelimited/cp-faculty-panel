...
interface EditTestModalProps {
  test: Test;
  onClose: () => void;
  onUpdate: (testId: string, payload: any) => Promise<any>;
}

const EditTestModal: React.FC<EditTestModalProps> = ({ test, onClose, onUpdate }) => {
  ...
  const parseHmsStringToSeconds = (hms: string) => {
    if (!hms) return 0;
        ...
        <div className="sticky top-0 bg-white rounded-t-2xl border-b border-gray-200 px-8 py-6 flex justify-between items-center z-10">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-green-100 rounded-xl flex items-center justify-center">
              <BookOpen className="w-5 h-5 text-[#4CA466]" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-900">Edit Test</h2>
              <p className="text-sm text-gray-500">Modify test details and configuration</p>
            </div>
          </div>
          <button
            onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-6 rounded-xl">
                ...
                <p className="text-xs text-gray-500 mt-3">Duration is required and must be greater than zero.</p>
              </div>
            </div>
